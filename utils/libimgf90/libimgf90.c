...
void gdalcalcstats_(char *file, int filelen)
{
char *psz;

  psz = (char*)calloc( filelen + 1, sizeof(char) );
  
  strncpy( psz, file, filelen );
  
  gdalcommon_calcstats(psz);

  free( psz );
}

void gdalcalcstatsignore_(char *file,float *ignore,int filelen)
{
char *psz;
  ...
  gdalcommon_calcstatsignore(psz,*ignore);
